            ...
            <div className="flex flex-col items-center justify-between gap-6 lg:flex-row">
              <div className="flex gap-6 text-sm font-medium text-neutral-800 dark:text-neutral-200">
                {menu.map(({ name, href }, index) => (
                  <NavLink key={`footer_menu_${index}`} href={href}>
                    {name}
                  </NavLink>
                ))}
              </div>
              <p className="text-sm text-neutral-400 dark:text-neutral-500">
                &copy; {new Date().getFullYear()} Christopher Douglas. All
                rights reserved.
              </p>
            </div>
            <p className="mt-1 text-center text-xs text-neutral-400 lg:text-right dark:text-neutral-500">
