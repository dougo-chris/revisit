...
function extractKeywords(title, content) {
  ...
  const topics = {
    'AI/ML':
      /\b(ai|artificial intelligence|machine learning|ml|llm|claude|gpt|openai|chatgpt)\b/gi,
    Development:
      /\b(code|coding|developer|engineering|engineer|technical|tech stack|api|database|git|pull request)\b/gi,
    Business:
      /\b(business|revenue|sales|customer|market|startup|company|growth|scale|product)\b/gi,
    Career: /\b(career|job|hiring|recruit|interview|resume|linkedin|work)\b/gi,
    Leadership:
      /\b(leadership|leader|manage|management|team|culture|people)\b/gi,
    Learning:
      /\b(learn|learning|education|teach|course|book|podcast|masterclass)\b/gi,
    ...
    'Food/Lifestyle':
      /\b(food|restaurant|burger|lunch|dinner|recipe|kite|footy|jeans)\b/gi,
  }
...
`
  ...
  const topics = Object.entries(stats.byTopic).sort((a, b) => b[1] - a[1])
  ...
  // Multi-topic notes
  const multiTopicNotes = notes
    .filter((n) => n.topics.length > 2)
